...
import { AhmadMilanPage } from '../ahmad-milan/ahmad-milan';
...
@Component({
  selector: 'page-hobby-astro',
  templateUrl: 'hobby-astro.html',
  encapsulation: ViewEncapsulation.None
})
export class HobbyAstroPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public platform: Platform,  public horoService: HoroscopeService, private callNumber: CallNumber, public device: Device, private file: File, private toast: Toast) {
    this.info = 'Loading, please wait..';
	   this.horoService.getAllHobbyAsts()
	   .subscribe(res => {
  ...
  viewprof(event, ast) {
	...
	item.uid = ast.uid;
	 ...
	 {
		case 'ahmadmilan':
			this.navCtrl.push(AhmadMilanPage, {
			  item: item
			  });
			break;
		default:
