...
export class KpAstroPage implements OnInit {
  ...
  constructor(private router: Router, private translate: TranslateService, public shareService: ShareService, public platform: Platform, public renderer: Renderer2, public horoService: HoroscopeService, private device: Device ) {
    this.items = [
      { title: 'Planet & Significance', note: 'Know what each planet bestows based on its position', spin: false, show: true, fuse: true },
	  { title: 'Dasha & Transit predictions', note: 'Current Dasha & Transit predictions', spin: false, show: true, fuse: false },
	  { title: 'Lucky Days to sign an Agreement', note: 'Know the favourable days to sign an agreement', spin: false, show: true, fuse: false },
	  { title: 'Lucky Days for opening bank account', note: 'Know the favourable days for obtaining bank account', spin: false, show: true, fuse: false },
	  { title: 'To undergo a treatment', note: 'Know the favourable days to undergo health treatment', spin: false, show: true, fuse: true },
	  { title: 'Filing a court case', note: 'Know the favourable days for filing a court case', spin: false, show: true, fuse: false },
	  { title: 'To Occupy a new house', note: 'Know the favourable days to occupy a new house', spin: false, show: true, fuse: false },
	  { title: 'To take delivery of vehicle', note: 'Know the favourable days to take the new vehicle', spin: false, show: true, fuse: false },
   	  { title: 'To apply for passport', note: 'Know the favourable days to apply for passport', spin: false, show: true, fuse: false },
...
//console.log(svg);
 ...
 getHXY(h, w) {
	...
	btrmore()
	{
		if(this.rcft == 'Rectify Now...') this.router.navigate(['/btr'], {state: this.binf});
		else this.router.navigate(['/btr-info']);
	}
    hsignf(h)
    {
