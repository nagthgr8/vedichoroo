...
chartSel() {
 ...
 pur_handl() {
  ...
  {
	  //this.ncdts = cdts;
	  this.product.gpid = 'com.mypubz.eportal.126report';
	  if(this.paym == 'rpay') {
			this.razpay(126);
	  }
 ...
 paymSel(paym)
  ...
  razpay(amt) {
    ...
    apromo(event) {
		...
		this.pinfo = 'Applying..';
		if(this.promo.trim() != '') {
	    this.horoService.applyPromo(this.device.uuid, this.promo)
		   .subscribe(res => {
			   if(res['status'] == 'success') {
				   ...
				   this.pinfo = 'This PROMO CODE is expired';
			   } else {
				   this.pinfo = 'Invalid PROMO CODE';
			   }
			}, (err) => {
			});	  
		}
