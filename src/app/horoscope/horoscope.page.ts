...
export class HoroscopePage implements OnInit {
  ...
  moon_phase: string = '';
	...
	ast: any;
	preferDark: any;
  constructor(private router: Router, private zone: NgZone, private route: ActivatedRoute, public shareService: ShareService, public renderer: Renderer2, public platform: Platform, private menu: MenuController, public device: Device, public horoService: HoroscopeService, private translate: TranslateService)
  {
  ...
  ngOnInit() {
   ...
   this.platform.ready().then((readySource) => {
	...
	let asts = this.shareService.getASTS();
	console.log('asts', asts);
	console.log('asts after sort', asts);
	this.ast = asts[0];
...
//		console.log('birthstar', this.bstar);
	...
	//	return nak.name + '|' + nak.location.start.split(',')[0] + '|' + nak.ruler + '|' + nak.location.start.split(',')[1] + '|' + nak.location.end.split(',')[1];
	   if(this.binf.vims != null) {
	      this.vimsObj(this.binf.vims);	   
				...
				.subscribe(res => {
					console.log('setVIMS', this.binf.dob);
	...
	publishRep()
	{
		...
		h_code += '</span>';
		if(this.shareService.getLANG().toLowerCase() == 'en') {
			h_code += "<h3>Your functional malefic planets are : </h3>";
		} else if(this.shareService.getLANG().toLowerCase() == 'te') {
			h_code += "<h3>మీ క్రియాత్మక హానికర గ్రహాలు </h3>";
		} else if(this.shareService.getLANG().toLowerCase() == 'hi') {
			h_code += "<h3>आपके कार्यशील अशुभ ग्रह हैं: </h3>";
		} else if(this.shareService.getLANG().toLowerCase() == 'ta') {
			h_code += "<h3>உங்கள் செயல்பாட்டில் உள்ள தீங்கான கிரகங்கள்</h3>";
		}
		h_code += '<span>';
		if (this.trikona_lords.length > 0) {
			var lords = this.fm_lords.split('\|');
			for (i = 0; i < lords.length; i++) {
				h_code += ' ' + this.shareService.translate_func(lords[i]);
			}
		}
		...
		if(this.binf.retro != '') {
			console.log('retro', this.binf.retro);
			var rpls = this.binf.retro.split(',');
			let retpls: string = '';
			for(var r = 0; r < rpls.length; r++) {
				if (rpls[r] != '' && rpls[r] != 'Ur' && rpls[r] != 'Pl' && rpls[r] != 'me' && rpls[r] != 'os' && rpls[r] != 'Ne' && rpls[r] != 'tr') 
					retpls += ' ' + this.ruler_name_v[rpls[r].toLowerCase()];
			}
			if(retpls != '') {
				h_code += '<h3>Retrograde Planet(s)</h3><span>' + retpls + '</span>';
			}
		}
		...
		for(var i = 0; i < 12; i++) {
				...
				if (plps.hasOwnProperty(sign)) {
					...
					for (var k = 0; k < pls.length; k++) {
						...
						console.log('plx', (hcord[0]-isz).toString());
						console.log('ply', s8.toString());
						console.log('pl', plt);
						console.log('plc', plc);
				...
				for (var j = i + 1; j < 12; j++) {
					...
					this.rashis_v[this.o_signs_v[j]] = (as).toString() + '|' + this.rashis_v[this.o_signs_v[j]].split('\|')[1];
						if (as == 5 || as == 9) {
							...
							this.fm_lords = this.fm_lords + '|' + this.rashi_lords_v[this.o_signs_v[j]];
						}
				}
				for (k = 0; k < i; k++) {
					var hno = ((12 - i) + (k + 1));
 ...
 more()
	{
	this.router.navigate(['/astrologers', {state: 'Talk to Astrologer'}]);
	}	
...
//	let y2: number = 0;
 ...
 atalk() {
	this.horoService.talkToAstro(this.ast.uid, this.device.uuid+ '|' + this.shareService.getCLAT().toString() + ',' + this.shareService.getCLNG().toString(), 'T')
	   .subscribe(res => {
	   }, (err) => {
	     console.log(err);
	   });	 
	
	window.location.href = 'https://wa.me/'  + this.ast.mob + '?text=I%20am%20interested%20to%20consult%20you%20for%20astrology';
 }
	rightmenu() {
