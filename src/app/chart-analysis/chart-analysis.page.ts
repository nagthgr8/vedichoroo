...
export class ChartAnalysisPage implements OnInit, AfterViewInit {
  ...
  constructor(private router: Router, public renderer: Renderer2, el: ElementRef, public platform: Platform, public shareService: ShareService, public horoService: HoroscopeService) { 
			...
			for (var j = 0; j < numberPerSide; j++) {
				...
				if (plps.hasOwnProperty(sign)) {
					...
					for (var k = 0; k < pls.length; k++) {
						...
						this.renderer.setAttribute(text, "font-weight", "bold");
						if(id == 2 && sign == this.hasc_sign) {
							this.renderer.addClass(text, "redText");
						} else if(pls[k].split(' ')[1] == 'AC') {
							this.renderer.addClass(text, "redText");
						}
						if(pls[k].split(' ')[1] == 'Mo') this.renderer.addClass(text, "blueText");
