...
export class NakInfoPage implements OnInit {
  ...
  ngOnInit() {
	let nakn = this.router.getCurrentNavigation().extras.state;
	this.platform.ready().then(() => {
	console.log(nakn);
		for(var i = 0; i < Object.keys(this.nakshatras_v).length; i++)
		{
			if(this.nakshatras_v[i].name == nakn.toLowerCase()) {
					this.nak = this.nakshatras_v[i];
					break;
			}
		}
		
  	   this.horoService.getJson('assets/data/' + nakn + '.json')
			.subscribe(res => {
				...
				this.nakinfo += '<h4>If Ascendant in ' + nakn + '</h4>';
				this.nakinfo += res['asc'];
				this.nakinfo += '<h4>If Moon in ' + nakn + '</h4>';
