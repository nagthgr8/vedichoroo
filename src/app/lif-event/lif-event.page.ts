...
export class LifEventPage implements OnInit {
  ...
  lid: number = -1;
  hcal: any;
  constructor(private router: Router, public platform: Platform, private horoService: HoroscopeService, public translate: TranslateService, private shareService: ShareService, private file: File, public eleRef: ElementRef, public renderer: Renderer2) { 
       platform.ready().then(() => {
  ...
  showCal(m,y)
  {
	...
	this.horoService.calForMon(m, y, this.clat+'|'+this.clng, this.localtz, ayanid)
		...
		this.publishReport(res);
						if(this.lid == 1) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates for signing an agreement, however one should also make sure the time where the sublord is also favouring. <br><span class="blue">Please tap on the date to get more details.</span></p>';
						} else if(this.lid == 2) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates for opening a bank account, however one should also make sure the time where the sublord is also favouring. <br><span class="blue">Please tap on the date to get more details.</span></p>';
						} else if(this.lid == 3) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates for undergoing any heath treatment. And is also important to pick the timing where sublord is also favourable. <br><span class="blue">Please tap on the date to get more details.</span></p>';
						} else if(this.lid == 4) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates for occupying a new house. And it is also very important to fix the time which is also suitable. <br><span class="blue">Please tap on each of favourable date in above calendar to know more.</span></p>';
						} else if(this.lid == 5) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates to take the delivery of new vehicle. </p>';
						} else if(this.lid == 6) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates to apply for oourt case.</p>';
						} else if(this.lid == 7) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates to apply for passport.</p>';
						}else if(this.lid == 8) {
							this.info2 = '<p>Based on your horoscope & current planetary transit, in the above calendar wherever <span class="pink">PINK</span> circles are shown are the good dates to apply for college admission.</p>';
						}
	 }, (err) => {
		this.info = err;
	});
